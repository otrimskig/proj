...
seq(as.Date("2025/01/01"), as.Date("2025/12/31"), "days")%>%
  as_tibble()%>%
  rename(date = value)%>%
  left_join(pps)%>%
...
write_rds(pp_year2, "pp_year25.rds")



###!change year
range_write(pp_year2,
            ss= "https://docs.google.com/spreadsheets/d/1tKUUyoX9Gufif4kHrN0rivxfPp6jSnzN6Qzv7nPiw0A/edit?gid=395264282#gid=395264282",
            sheet = "day25",
            range = "A3",
            col_names = FALSE,
            reformat = FALSE)

...
#rounds to nearest minute, for easier filtering (see next step)
read_sheet("https://docs.google.com/spreadsheets/d/1EMKP1T11kPVPc2-wmztRzEPNngxkw0rcAkpz09I3rFA/edit#gid=1911378700", 
           sheet = "corrections")%>%
  select(1,2)%>%
  mutate(in_time = round_date(in_time, unit = "minute"))%>%
  mutate(out_time = round_date(out_time, unit = "minute"))->corrections
...
otc_year3%>%
  ...
  #removes original, incorrect in_time that was corrected manually.
  anti_join(corrections, by="in_time")%>%
